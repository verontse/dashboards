...
interval_minutes = int(INTERVALS[selected_interval][:-1])  # Extract minutes from interval
...
# Fetch and calculate data functions remain the same
...
if selected_symbols:
    ...
    num_cols = min(len(selected_symbols), 2)  # Max 2 columns
    num_rows = (len(selected_symbols) + 1) // 2  # Calculate rows needed
            ...
            if idx < len(selected_symbols):
                ...
                if symbol in data_dict:
                    with cols[col]:
                        ...
                        strength = calculate_strength(data, weights)  # Pass weights to the function
    ...
    colors = ['blue', 'red', 'green', 'purple']  # Colors for different symbols
